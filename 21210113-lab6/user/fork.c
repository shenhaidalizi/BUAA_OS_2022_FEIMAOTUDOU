...
pgfault(u_int va)
{
	...
	if(ret < 0)user_panic("pgfault alloc falied!\n");
	//copy the content
	user_bcopy(va, tmp, BY2PG);
	//map the page on the appropriate place
	ret = syscall_mem_map(0, tmp, 0, va, PTE_V | PTE_R);
	if(ret < 0)user_panic("pgfault remap falid\n");
	//unmap the temporary place
	ret = syscall_mem_unmap(0, tmp);
	if(ret < 0)user_panic("pgfault unmap failed\n");
	return;
}
...
fork(void)
{
	...
	if(ret < 0)user_panic("fork alloc mem failed");
	ret = syscall_set_pgfault_handler(newenvid, __asm_pgfault_handler, UXSTACKTOP);
	if(ret < 0)user_panic("fork set pgfault failed");
	ret = syscall_set_env_status(newenvid, ENV_RUNNABLE);
	if(ret < 0)user_panic("fork set status failed");
